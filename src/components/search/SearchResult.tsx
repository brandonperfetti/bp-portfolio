...
const HitCount = connectStateResults(({ searchState, searchResults }) => {
  ...
  return validQuery ? (
    <div className="float-right flex">
      <div className="py-2 text-sm text-zinc-800 dark:text-white">
...
const PageHit = connectStateResults(({ searchState, searchResults, hit }) => {
  ...
  return Number(hitCount) > 0 ? (
    <div className="py-1">
      <div className="m-1 grid rounded-md p-3 shadow-md ring-1 ring-slate-200 transition hover:shadow-xl hover:ring-teal-500 dark:hover:ring-1">
...
const HitsInIndex: FC<{ index: any }> = ({ index }) => {
  return (
    <div className="py-2">
      <HitCount />
      <div className="max-h-[calc(100vh-18rem)] w-full overflow-scroll">
