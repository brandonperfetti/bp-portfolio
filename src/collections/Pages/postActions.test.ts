...
const shareTargetsAdd = findField(Pages.fields, 'shareTargetsAdd') as
  SelectField | undefined
const shareTargetsRemove = findField(Pages.fields, 'shareTargetsRemove') as
  SelectField | undefined
const ogImageMode = findField(Pages.fields, 'ogImageMode') as
  SelectField | undefined
const disableSharing = findField(Pages.fields, 'disableSharing')

/**
 * Per-entry post-action overrides on Pages (Batch 1 / T1, #51): unique enum
 * ids so the join tables never collide with the Posts/site copies, and the
 * additive-safe defaults every existing row back-fills.
 */
describe('pages post-actions schema', () => {
  it('names the per-collection share-target enums explicitly, within 63 chars', () => {
    expect(shareTargetsAdd?.enumName).toBe('enum_pages_share_targets_add')
    expect(shareTargetsRemove?.enumName).toBe('enum_pages_share_targets_remove')
    for (const name of [
  ...
  it('offers ogImageMode auto|bespoke|generated, defaulting to auto', () => {
    expect(ogImageMode?.enumName).toBe('enum_pages_og_image_mode')
