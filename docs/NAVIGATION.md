...
**Two reservation rules, deliberately not merged.**

| Set                                                                          | Where      | Effect                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RESERVED_PAGE_SLUGS` (`src/lib/cms/pagesRepo.ts`)                           | emit/serve | A **one-segment** path in this set is never served, emitted in the sitemap, or statically generated by the catch-all, because a dedicated route owns it. A Pages document at that path is legitimate and load-bearing — `/about`, `/tech`, `/uses`, `/projects`, `/corvus` and `/articles` all take their copy from one via `getCmsPageByPath`. |
| `CODE_OWNED_FIRST_SEGMENTS` (`src/collections/Pages/hooks/pageHierarchy.ts`) | save-time  | A path whose **first segment** is `api`, `admin`, `next`, or a metadata route (`feed.xml`, `llms.txt`, `llms-full.txt`, `sitemap.xml`, `robots.txt`, `manifest.webmanifest`) is rejected at validation. Nothing behind these can ever render.                                                                                                   |
...
**What a save rejects.** `beforeValidate` refuses a page that is its own parent,
a parent inside the page's own subtree (cycle), a path deeper than 3 segments
(Brandon, D3), a code-owned first segment, a path another page already serves,
and a path colliding with a Post's `/articles/…` URL — the last because Pages
and Posts are separate tables and no unique index can span them.

**Known limits, both tracked.**

- **Moving a parent does not cascade to its descendants.** Their stored paths
  stay stale until they are themselves saved. Deliberate: the cascade needs the
  redirect fan-out that extends #120, and landing one without the other would
  move a subtree of live URLs with nothing preserving the old ones (#150).
- **`src/lib/cms/pageMetadata.ts` still builds the generated-OG URL from
  `page.slug`.** `/api/og/page/[...segments]` is already path-keyed and every
  existing one-segment URL works unchanged, but a _nested_ page's card is not
  yet requested at its full path.
